...
export default function LogisticGrowthSketch({ width = 500, height = 380 }) {
        ...
        >
            {(p) => {
                ...
                function drawAxes() {
                    ...
                    p.rotate(-p.HALF_PI);
                    p.textAlign(p.CENTER, p.TOP);
                    p.text("Population (xₙ)", 0, 0);
                    p.pop();
                }

                function drawCurve(prog) {
                    // Draw curve left->right up to n = prog*maxN
                    ...
                    for (let i = 0; i <= endSamples; i++) {
                        ...
                        // Compute x value at fractional n using the logistic map
                        let x = x0;
                        let fullSteps = Math.floor(n);
                        for (let step = 0; step < fullSteps; step++) {
                            x = x + r * (1 - x) * x;
                        }
                        // Interpolate partial step if needed
                        let frac = n - fullSteps;
                        if (frac > 0) {
                            let xNext = x + r * (1 - x) * x;
                            x = p.lerp(x, xNext, frac);
                        }

                        p.vertex(mapNtoX(n), mapXtoY(x));
                    }
                    p.endShape();
                }
                ...
                function mapXtoY(xVal) {
                    // Map x value to y coordinate
                    let minX = 0;
                    let maxX = 1.6;

                    let t = (xVal - minX) / (maxX - minX);
                    t = p.constrain(t, 0, 1);
