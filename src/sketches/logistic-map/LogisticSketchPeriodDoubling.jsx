...
export default function LogisticMapPeriodDoubling({ width = 500, height = 380 }) {
    ...
    >
      {(p) => {
        ...
        p.setup = function () {
          ...
          rSlider = p.createSlider(1.8, 2.8, r, 0.01);
        ...
        function drawAxes() {
          ...
          for (let n = 0; n <= maxN; n++) {
            if (n % 5 !== 0) continue; // only label every 5th month
            const x = mapNtoX(n);
            p.stroke(255, 70);
            p.strokeWeight(1);
            p.line(x, y0, x, y0 + 6);
